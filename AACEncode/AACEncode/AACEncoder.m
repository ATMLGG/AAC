...
- (instancetype)init {
    if (self = [super init]) {
        _encoderQueue = dispatch_queue_create("AAC Encoder", DISPATCH_QUEUE_SERIAL);
        _callbackQueue = dispatch_queue_create("AAC Encoder Callback", DISPATCH_QUEUE_SERIAL);
...
- (void)encodeSampleBuffer:(CMSampleBufferRef)sampleBuffer completionBlock:(void (^)(NSData *, NSError *))completionBlock {
    ...
    dispatch_async(_encoderQueue, ^{
        ...
        outAudioBufferList.mBuffers[0].mNumberChannels = 1;                 //声道数
        outAudioBufferList.mBuffers[0].mDataByteSize = (int)_aacBufferSize; //缓冲区大小
        outAudioBufferList.mBuffers[0].mData = _aacBuffer;                  //缓冲区
...
- (void) setupEncoderFromSampleBuffer:(CMSampleBufferRef)sampleBuffer {
    ...
    AudioStreamBasicDescription outAudioStreamBasicDescription = {0}; // 初始化输出流的结构体描述为0. 很重要。只需要这样写就行
    ...
    AudioClassDescription *description = [self
                                          ...
                                          fromManufacturer:kAppleSoftwareAudioCodecManufacturer]; //软编
                                                         //kAppleHardwareAudioCodecManufacturer   //硬编
